...
void qmp_marshal_user_def_cmd2(QDict *args, QObject **ret, Error **errp)
{
    ...
    retval = qmp_user_def_cmd2(arg.ud1a, arg.ud1b, &err);
    if (err) {
        error_propagate(errp, err);
        goto out;
    }
...
void qmp_marshal_test_if_cmd(QDict *args, QObject **ret, Error **errp)
{
    ...
    if (visit_type_TestIfStruct_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    retval = qmp_test_if_cmd(&arg, &err);
    if (err) {
        error_propagate(errp, err);
        goto out;
    }
...
out:
    ...
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_TestIfStruct_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
...
void qmp_marshal_test_features0(QDict *args, QObject **ret, Error **errp)
{
    ...
    retval = qmp_test_features0(arg.fs0, arg.fs1, arg.fs2, arg.fs3, arg.fs4, arg.cfs1, arg.cfs2, arg.cfs3, arg.cfs4, &err);
    if (err) {
        error_propagate(errp, err);
        goto out;
    }
