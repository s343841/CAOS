...
void coroutine_fn qmp_marshal_screendump(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_screendump(arg.filename, arg.device, arg.has_head, arg.head, arg.has_format, arg.format, &err);
    if (err) {
...
void qmp_marshal_input_send_event(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_input_send_event(arg.device, arg.has_head, arg.head, arg.events, &err);
    if (err) {
...
void qmp_marshal_display_update(QDict *args, QObject **ret, Error **errp)
{
    ...
    bool ok = false;
    Visitor *v;
    DisplayUpdateOptions arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_DisplayUpdateOptions_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_DISPLAY_UPDATE)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_display_update(req_json->str);
    }

    qmp_display_update(&arg, &err);
    if (err) {
        trace_qmp_exit_display_update(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    trace_qmp_exit_display_update("{}", true);

out:
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_DisplayUpdateOptions_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
...
void qmp_marshal_client_migrate_info(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_client_migrate_info(arg.protocol, arg.hostname, arg.has_port, arg.port, arg.has_tls_port, arg.tls_port, arg.cert_subject, &err);
    if (err) {
