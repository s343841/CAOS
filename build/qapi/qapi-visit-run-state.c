...
bool visit_type_StatusInfo_members(Visitor *v, StatusInfo *obj, Error **errp)
{
    ...
    if (visit_policy_reject(v, "singlestep", 1u << QAPI_DEPRECATED, errp)) {
        return false;
    }
    if (!visit_policy_skip(v, "singlestep", 1u << QAPI_DEPRECATED)) {
...
{
    bool has_info = !!obj->info;

    if (!visit_type_GuestPanicAction(v, "action", &obj->action, errp)) {
        return false;
    }
    if (visit_optional(v, "info", &has_info)) {
