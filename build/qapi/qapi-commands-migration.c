...
void qmp_marshal_set_vcpu_dirty_limit(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    q_obj_set_vcpu_dirty_limit_arg arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_q_obj_set_vcpu_dirty_limit_arg_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_SET_VCPU_DIRTY_LIMIT)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_set_vcpu_dirty_limit(req_json->str);
    }

    qmp_set_vcpu_dirty_limit(arg.has_cpu_index, arg.cpu_index, arg.dirty_rate, &err);
    if (err) {
        trace_qmp_exit_set_vcpu_dirty_limit(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    trace_qmp_exit_set_vcpu_dirty_limit("{}", true);

out:
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_q_obj_set_vcpu_dirty_limit_arg_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}

void qmp_marshal_cancel_vcpu_dirty_limit(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    q_obj_cancel_vcpu_dirty_limit_arg arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_q_obj_cancel_vcpu_dirty_limit_arg_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_CANCEL_VCPU_DIRTY_LIMIT)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_cancel_vcpu_dirty_limit(req_json->str);
    }

    qmp_cancel_vcpu_dirty_limit(arg.has_cpu_index, arg.cpu_index, &err);
    if (err) {
        trace_qmp_exit_cancel_vcpu_dirty_limit(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    trace_qmp_exit_cancel_vcpu_dirty_limit("{}", true);

out:
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_q_obj_cancel_vcpu_dirty_limit_arg_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}

static void qmp_marshal_output_DirtyLimitInfoList(DirtyLimitInfoList *ret_in,
                                QObject **ret_out, Error **errp)
{
    Visitor *v;

    v = qobject_output_visitor_new_qmp(ret_out);
    if (visit_type_DirtyLimitInfoList(v, "unused", &ret_in, errp)) {
        visit_complete(v, ret_out);
    }
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_DirtyLimitInfoList(v, "unused", &ret_in, NULL);
    visit_free(v);
}

void qmp_marshal_query_vcpu_dirty_limit(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    DirtyLimitInfoList *retval;
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_QUERY_VCPU_DIRTY_LIMIT)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_query_vcpu_dirty_limit(req_json->str);
    }

    retval = qmp_query_vcpu_dirty_limit(&err);
    if (err) {
        trace_qmp_exit_query_vcpu_dirty_limit(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    qmp_marshal_output_DirtyLimitInfoList(retval, ret, errp);

    if (trace_event_get_state_backends(TRACE_QMP_EXIT_QUERY_VCPU_DIRTY_LIMIT)) {
        g_autoptr(GString) ret_json = qobject_to_json(*ret);

        trace_qmp_exit_query_vcpu_dirty_limit(ret_json->str, true);
    }
...
static void qmp_marshal_output_MigrationThreadInfoList(MigrationThreadInfoList *ret_in,
                                QObject **ret_out, Error **errp)
{
    Visitor *v;

    v = qobject_output_visitor_new_qmp(ret_out);
    if (visit_type_MigrationThreadInfoList(v, "unused", &ret_in, errp)) {
        visit_complete(v, ret_out);
    }
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_MigrationThreadInfoList(v, "unused", &ret_in, NULL);
    visit_free(v);
}

void qmp_marshal_query_migrationthreads(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    MigrationThreadInfoList *retval;
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_QUERY_MIGRATIONTHREADS)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_query_migrationthreads(req_json->str);
    }

    retval = qmp_query_migrationthreads(&err);
    if (err) {
        trace_qmp_exit_query_migrationthreads(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    qmp_marshal_output_MigrationThreadInfoList(retval, ret, errp);

    if (trace_event_get_state_backends(TRACE_QMP_EXIT_QUERY_MIGRATIONTHREADS)) {
        g_autoptr(GString) ret_json = qobject_to_json(*ret);

        trace_qmp_exit_query_migrationthreads(ret_json->str, true);
    }
