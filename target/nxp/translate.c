...
//SystemRegisters
static TCGv cpu_sysr[NXPA_SYS_REG];
...
void nxp_tcg_init(void){
    ...
    for(i = 0;i < NUM_REG_PAGE_SIZE; ++i) {
        cpu_r[i] = tcg_global_mem_new_i32(cpu_env,
                                          offsetof(CPUNXPAState, r[i]),
                                          nxp_cpu_r_names[i]);
    }

    for(i = 0;i < NXPA_SYS_REG; ++i) {
        char* name;
    ...
    for(i = 0;i < 32; ++i) {
        cpu_sflags[i] = tcg_global_mem_new_i32(cpu_env,
                                               offsetof(CPUNXPAState, sflags[i]),
                                               nxp_cpu_sr_flag_names[i]);
    }
}
...
static bool trans_ACALL(DisasContext *ctx, arg_ACALL *a){
    ...
    tcg_gen_addi_i32(cpu_r[NXPA_LR_REG], cpu_r[NXPA_LR_REG], 0x2);
    tcg_gen_add_i32(cpu_r[NXPA_PC_REG], temp, cpu_sysr[2]);
...
static bool trans_RCALL_f1(DisasContext *ctx, arg_RCALL_f1 *a){
    ...
    tcg_gen_addi_i32(cpu_r[NXPA_LR_REG], cpu_r[NXPA_PC_REG], 2);
    tcg_gen_addi_i32(cpu_r[NXPA_PC_REG], cpu_r[NXPA_PC_REG], disp);
...
static void nxp_tr_init_disas_context(DisasContextBase *dcbase, CPUState *cs){
    CPUNXPAState *env = cs->env_ptr;
...
static void nxp_tr_tb_start(DisasContextBase *db, CPUState *cs){

}

static void nxp_tr_insn_start(DisasContextBase *dcbase, CPUState *cs){
    DisasContext *ctx = container_of(dcbase, DisasContext, base);
    tcg_gen_insn_start(ctx->base.pc_next);
}

static void nxp_tr_translate_insn(DisasContextBase *dcbase, CPUState *cs){
    DisasContext *ctx = container_of(dcbase, DisasContext, base);
    uint32_t insn;
    ...
    if (!decode_insn(ctx, insn)) {
        error_report("[NXP-TCG] nxp_tr_translate_insn, illegal instr, pc: 0x%04x\n", ctx->base.pc_next);
        gen_helper_raise_illegal_instruction(cpu_env);
    }
}

static void nxp_tr_tb_stop(DisasContextBase *dcbase, CPUState *cs){
    DisasContext* ctx = container_of(dcbase, DisasContext, base);
    switch(ctx->base.is_jmp) {
        ...
        case DISAS_EXIT:
            ...
            break;
        default:
            printf("[nxp_tr_tb_stop] ERROR: undefined condition\n");
            g_assert_not_reached();
    }
}

static void nxp_tr_disas_log(const DisasContextBase *dcbase,
...
static const TranslatorOps nxp_tr_ops = {
        .init_disas_context = nxp_tr_init_disas_context,
        .tb_start           = nxp_tr_tb_start,
        .insn_start         = nxp_tr_insn_start,
        .translate_insn     = nxp_tr_translate_insn,
        .tb_stop            = nxp_tr_tb_stop,
        .disas_log          = nxp_tr_disas_log,
};
...
{
    DisasContext dc = { };
    translator_loop(cs, tb, max_insns, pc, host_pc, &nxp_tr_ops, &dc.base);
}