...
static uint32_t decode_insn_load_bytes(DisasContext *ctx, uint32_t insn,
                                       int i, int n){
    printf("[nxp-DISAS] decode_insn_load_bytes at: 0x%04x\n", ctx->pc);
    ...
    if((ctx->bytes[0] & NXP_EXTENDED_INSTR_FORMAT_MASK_LE) == NXP_EXTENDED_INSTR_FORMAT_MASK_LE)
    {
        ...
        ctx->addr += 4;
        printf("[nxp-DISAS] decode_insn_load_bytes, loaded (long): 0x%04x\n", insn);

    } else {
        insn = bfd_getb16(ctx->bytes) << 16;
        ctx->addr += 2;
        printf("[nxp-DISAS] decode_insn_load_bytes, loaded (short): 0x%04x\n", insn);
    }
...
int nxp_print_insn(bfd_vma addr, disassemble_info *dis)
{
    printf("[nxp-DISAS] nxp_print_insn\n");
    DisasContext ctx;
    uint32_t insn;
    int i;
...
#define REG(x) nxp_cpu_r_names[x]
